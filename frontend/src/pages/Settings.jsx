...
export default function Settings() {
  ...
  const loadAll = () => {
    ...
    api.get("/photographers")
      .then(({ data }) => {
        const list = Array.isArray(data) ? data : (data?.data || data?.photographers || []);
  ...
  const addPkg = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err?.response?.data?.detail || "Gagal menambahkan paket.");
    }
  };

  const savePkg = async (p) => {
    try {
      await api.put(`/packages/${p.package_id}`, { 
        name: p.name, 
  ...
  const addPho = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err?.response?.data?.detail || "Gagal menambahkan fotografer.");
    }
  };
  ...
  const savePho = async (p) => {
    try {
      ...
      toast.error("Gagal memperbarui fotografer");
    }
  };
  ...
  const delPho = async (id) => { 
    try {
      ...
      toast.error("Gagal menghapus fotografer");
    }
  };

  const updPkgField = (id, field, val) => {
    setPackages((arr) => (Array.isArray(arr) ? arr.map((x) => (x.package_id === id ? { ...x, [field]: val } : x)) : []));
  };

  const updPhoField = (id, field, val) => {
    setPhotographers((arr) => (Array.isArray(arr) ? arr.map((x) => (x.photographer_id === id ? { ...x, [field]: val } : x)) : []));
  };
          ...
          {safePackages.map((p) => (
            <div key={p.package_id} data-testid={`package-row-${p.package_id}`} className="rounded-lg border border-moss-900/10 bg-white p-5">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5 items-end">
                <div><Label className="label-xs">Nama</Label><Input value={p.name || ""} onChange={(e) => updPkgField(p.package_id, "name", e.target.value)} className="h-10 mt-1.5" /></div>
                <div><Label className="label-xs">Harga</Label><Input type="number" value={p.price || ""} onChange={(e) => updPkgField(p.package_id, "price", e.target.value)} className="h-10 mt-1.5" /></div>
                <div><Label className="label-xs">DP</Label><Input type="number" value={p.dp_amount || ""} onChange={(e) => updPkgField(p.package_id, "dp_amount", e.target.value)} className="h-10 mt-1.5" /></div>
                <div><Label className="label-xs">Deskripsi</Label><Input value={p.description || ""} onChange={(e) => updPkgField(p.package_id, "description", e.target.value)} className="h-10 mt-1.5" /></div>
                <div className="flex items-center gap-2">
                  <Switch checked={!!p.active} onCheckedChange={(v) => updPkgField(p.package_id, "active", v)} data-testid={`package-active-${p.package_id}`} />
              ...
              <p className="text-xs text-muted-foreground mt-3">Harga tampil di form: {rupiah(p.price || 0)} · DP {rupiah(p.dp_amount || 0)}</p>
            </div>
          ))}
        </TabsContent>
